...
class AIAgent:
    ...
    def run_conversation(
        ...
        if (
            self.compression_enabled
            and self.context_compressor.can_compress(messages)
        ):
        ...
        while api_call_count < self.max_iterations and self.iteration_budget.remaining > 0:
            ...
            try:
                ...
                if assistant_message.tool_calls:
                    ...
                    # Show spinner during tool execution so user sees activity
                    _tc_labels = ", ".join(tc.function.name for tc in assistant_message.tool_calls)
                    if self.thinking_callback:
                        self.thinking_callback(f"🔧 {_tc_labels}...")

                    try:
                        self._execute_tool_calls(assistant_message, messages, effective_task_id, api_call_count)
                    finally:
                        # Clear tool spinner after execution completes
                        if self.thinking_callback:
                            self.thinking_callback("")
