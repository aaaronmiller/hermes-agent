...
def _fmt_duration(seconds) -> str:
    """Format a duration in seconds to a compact string (e.g., '2h 15m', '45s')."""
    if seconds is None:
        return "—"
    try:
        secs = float(seconds)
...
"""
    ...
    sessions_list.add_argument("--verbose", "-v", action="store_true", help="Show detailed columns (tokens, cost)")
    sessions_list.add_argument("--all", action="store_true", help="Show all sources (including cron, tool)")
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Default: show only 'cli' interactive sessions, not cron/tool sessions
            default_source = None if show_all else "cli"
            source = args.source or default_source
            # Exclude logic: hide 'tool' and 'cron' sources unless --source explicitly set or --all
            exclude = None if (source or show_all) else ["cron", "tool"]
            sessions = db.list_sessions_rich(source=source, limit=args.limit, include_tool_errors=True, exclude_sources=exclude)
            if not sessions:
                print("No sessions found.")
                return
            ...
            if not verbose:
                # Show active filter
                if source:
                    print(f"[Source: {source}] {len(sessions)} session(s)")
                else:
                    print(f"[All sources] {len(sessions)} session(s)")

            if verbose:
                # Wide format with full stats
                print(f"{'ID':>16} {'Title':<24} {'Src':<6} {'Turns':>5} {'Tools':>5} {'Err':>4} {'In':>6} {'Out':>6} {'Cost':>6} {'Dur':>8} {'Last Active':<12} {'Preview'}")
                print("═" * 140)
                for s in sessions:
                    sid = s["id"][:16]
                    title = (s.get("title") or "—")[:24]
                    src = s["source"][:6]
                    turns = max(0, s.get("message_count", 0) // 2)
                    tools = s.get("tool_call_count", 0)
                    errs = s.get("tool_errors", 0)
                    inp = _fmt_tokens(s.get("input_tokens"))
                    out = _fmt_tokens(s.get("output_tokens"))
                    cost = s.get("actual_cost_usd")
                    cost_str = f"${cost:.2f}" if cost and cost > 0 else "—"
                    dur = _fmt_duration(s.get("duration", 0))
                    last = _relative_time(s.get("last_active", s.get("started_at")))
                    preview = (s.get("preview", "") or "")[:45]
                    print(f"{sid:>16} {title:<24} {src:<6} {turns:>5} {tools:>5} {errs:>4} {inp:>6} {out:>6} {cost_str:>6} {dur:>8} {last:<12} {preview}")

            else:
                # Compact format: always show source to distinguish cron/interactive,
                # and dynamically size the label column to fit the terminal.
                try:
                    import shutil
                    term_width = shutil.get_terminal_size().columns
                except Exception:
                    term_width = 140
                fixed = 6 + 5 + 5 + 4 + 8 + 12 + 6
                label_width = max(20, term_width - fixed)
                header = f"{'Title (or Preview)':<{label_width}} {'Src':<6} {'Turns':>5} {'Tools':>5} {'Err':>4} {'Dur':>8} {'Last Active':<12} ID"
                print(header)
                print("─" * len(header))
                for s in sessions:
                    turns = max(0, s.get("message_count", 0) // 2)
                    ...
                    src = s.get("source", "")[:6]
                    sid = s["id"]
                    label = s.get("title") or s.get("preview") or "—"
                    label = label[:label_width]
                    print(f"{label:<{label_width}} {src:<6} {turns:>5} {tools:>5} {errs:>4} {dur:>8} {last:<12} {sid}")
        elif action == "export":
            if args.session_id:
            ...
            # By default, exclude both cron and tool sessions to focus on interactive sessions
            _browse_exclude = None if source else ["cron", "tool"]
